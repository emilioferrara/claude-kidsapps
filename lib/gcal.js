...
function toGoogleEvent(event) {
  const gEvent = {
    ...
    description: event.member_name ? `For: ${event.member_emoji || ''} ${event.member_name}` : undefined,
  };
...
function fromGoogleEvent(gEvent) {
  ...
  if (gEvent.start.dateTime) {
    ...
    if (gEvent.end.dateTime) {
      end_time = new Date(gEvent.end.dateTime).toTimeString().slice(0, 5);
    }
  } else {
  ...
  return {
    ...
    recurrence: null, // singleEvents=true, so no recurrence to parse
    member_id: null,
...
async function pushAllEvents(db, saveDb) {
  ...
  const events = queryAll(db,
    `SELECT e.*, f.name as member_name, f.emoji as member_emoji
     FROM events e LEFT JOIN family_members f ON e.member_id = f.id
